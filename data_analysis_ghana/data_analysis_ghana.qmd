...
# Skip country plot since we're filtering for Ghana only
# The country plot is not relevant for a single-country analysis
```
...
ggsave(here::here("data_analysis_ghana/outputs/plot_population_pyramid.png"), plot_pyramid,
...
# Save the plot at 300 dpi
ggsave(here::here("data_analysis_ghana/outputs/plot_education_stacked.png"), plot_education_stacked,
...
# Save the plot at 300 dpi
ggsave(here::here("data_analysis_ghana/outputs/plot_employment_stacked.png"), plot_employment_stacked,
       width = 10, height = 3, dpi = 300)
```
...
prog_labels <- label_dict |>
  ...
  select(name, label) |>
  mutate(label = case_when(
    str_detect(label, "I have none") ~ "I have\nnone",
...
plot_programming <- prog_data |>
  count(skill_type, label, .drop = FALSE) |>
  ggplot(aes(x = reorder(label, n), y = n, fill = skill_type)) +
...
# Save the plot at 300 dpi
ggsave(here::here("data_analysis_ghana/outputs/plot_programming_skills.png"), plot_programming,
       width = 8, height = 6, dpi = 300)
```
...
# Save the plot at 300 dpi
ggsave(here::here("data_analysis_ghana/outputs/plot_llm_platforms.png"), plot_llm_platforms,
       width = 8, height = 6, dpi = 300)
```
