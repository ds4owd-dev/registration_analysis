...
# Fetch roster from GitHub with token authentication
github_token <- Sys.getenv("GITHUB_PAT")
if (github_token == "") {
  stop("GITHUB_PAT environment variable not set. Please add it to your .Renviron file.")
}

roster_url <- "https://raw.githubusercontent.com/ds4owd-dev/admin/refs/heads/main/data/final/tbl-01-ds4owd-participant-roster.csv"
...
# read participant roster with authentication
team_users <- c("massarin", "larnsce", "seawaR",
                "galacticasparagus", "rainbow-train", "betadetective")

# Create curl handle with authorization header
h <- curl::new_handle()
curl::handle_setheaders(h, "Authorization" = paste("token", github_token))

participant_roster <- read_csv(curl::curl(roster_url, handle = h), show_col_types = FALSE) |>
...
ggsave(here::here("output/registration_plots/countries_plot.jpg"), plot = countries_plot, width = 8, height = 4, dpi = 300)
```
...
ggsave(here::here("output/registration_plots/plot_pyramid.jpg"), plot = plot_pyramid, width = 8, height = 6, dpi = 300)
```
