...
We're thrilled to have received **`r nrow(registration_data)` registrations** so far from passionate learners worldwide who are ready to transform water, sanitation, and hygiene data into actionable insights! 📊
...
**🗺 Global reach, diverse perspectives:** Our participants span continents, with African countries leading the way. We have `r top1_count` registrations from `r top1_country`, followed by strong representation from `r top2_country` (`r top2_count` registrations) and `r top3_country` (`r top3_count` registrations). Also very exciting to see a strong female participation with `r female_percent`% of the registrations. You're creating an inclusive learning environment where diverse voices will enrich our experience. 🤝
...
# Get labels for both age and gender
age_labels_pyramid <- label_dict |> 
  filter(list_name == "list_age_group") |> 
  select(name, label)

gender_labels_pyramid <- label_dict |> 
  filter(list_name == "list_gender") |> 
  select(name, label)

# Prepare data for population pyramid - collapse older age groups
pyramid_data <- registration_data |> 
  ...
  count(age_group_collapsed, gender) |> 
  left_join(age_labels_pyramid, by = c("age_group_collapsed" = "name")) |> 
  left_join(gender_labels_pyramid, by = c("gender" = "name")) |> 
  mutate(age_label = case_when(
    age_group_collapsed == "55_older" ~ "55 or older",
    TRUE ~ coalesce(label.x, age_group_collapsed)
  ),
...
plot_pyramid <- pyramid_data |> 
  ...
  geom_col(width = 0.8) +
  geom_text(aes(label = abs(n_adjusted), 
...
# Create alternative stacked bar plot for education level
education_stacked_data <- registration_data |> 
  count(education) |> 
  left_join(education_labels, by = c("education" = "name")) |> 
  mutate(education_label = coalesce(label, education),
         # Collapse categories with count < 5 into Other
         education_final = ifelse(n < 5, "Other", education_label)) |> 
...
# Create labels with both category name and percentage
education_stacked_data <- education_stacked_data |> 
  mutate(percent = round(n/sum(n) * 100),
...
# Create alternative stacked bar plot for employment situation
employment_stacked_data <- registration_data |> 
  ...
  summarise(n = sum(n)) |> 
  ungroup() |> 
  # Then collapse categories with count < 10 into Other
...
# Create labels with both category name and percentage
employment_stacked_data <- employment_stacked_data |> 
  mutate(percent = round(n/sum(n) * 100),
